...
<div class="container py-4">
    ...
    <div class="card shadow-sm">
        ...
        <div class="card-body">
            <form action="{{ route('admin.cme.store') }}" method="POST">
                @csrf
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="name" name="name" 
                                value="{{ old('name', 'Groupe CME ' . \Carbon\Carbon::now()->format('d/m/Y')) }}" required>
                            @error('name')
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control" id="session_date" name="session_date" 
                                value="{{ old('session_date', \Carbon\Carbon::now()->addDay()->format('Y-m-d')) }}" 
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="session_time" name="session_time" required>
                                <option value="morning" {{ old('session_time') == 'morning' ? 'selected' : '' }}>Matin (9h-12h)</option>
                                <option value="afternoon" {{ old('session_time') == 'afternoon' ? 'selected' : '' }}>Après-midi (14h-17h)</option>
                            </select>
                            @error('session_time')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                                    ...
                                    <td>
                                        <div class="form-check">
                                            <input class="form-check-input candidate-check" type="checkbox" 
                                                ...
                                                {{ in_array($candidate->id, old('candidates', [])) ? 'checked' : '' }}>
                                        </div>
                                    </td>
                ...
                <div class="d-grid gap-2">
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save"></i> Créer le groupe CME
                    </button>
                </div>
            </form>
        </div>
